...
from ..models import AnalyzedComments, ThreadContext
...
{self._indent_text(escaped_persona, 4)}
  ...
  <execution_priority>
    ...
    def _format_execution_instructions(self, analyzed_comments: AnalyzedComments) -> str:
        ...
        if primary_tasks:
            ...
            for task in primary_tasks:
                ...
                task_attrs = (
                    f"priority='{task['priority']}' comment_id='{task.get('comment_id', 'N/A')}'"
                )
                ...
                if task.get("has_ai_prompt") and task.get("ai_prompt_content"):
                    ...
                    instructions.append(
                        f"{self._indent_text(escape(task['ai_prompt_content']), 8)}"
                    )
                    instructions.append("      </ai_agent_prompt>")
    ...
    def _format_context_data(self, analyzed_comments: AnalyzedComments) -> str:
        ...
        if analyzed_comments.summary_comments:
            ...
            for summary in analyzed_comments.summary_comments:
                ...
                if hasattr(summary, "walkthrough") and summary.walkthrough:
                    sections.append(
                        f"      <walkthrough>{escape(summary.walkthrough)}</walkthrough>"
                    )
                sections.append("    </summary>")
            sections.append("  </summary_information>")
        ...
        if analyzed_comments.unresolved_threads:
            ...
            for thread in analyzed_comments.unresolved_threads:
                sections.append(
                    f"    <thread id='{thread.thread_id}' resolved='{thread.is_resolved}'>"
                )
                sections.append(f"      <file_context>{escape(thread.file_context)}</file_context>")
                if thread.line_context:
                    sections.append(
                        f"      <line_context>{escape(thread.line_context)}</line_context>"
                    )
                ...
                if inline_comments:
                    ...
                    for comment in inline_comments:
                        ...
                        sections.append(
                            f"          <content>{escape(comment['content'])}</content>"
                        )
                        if comment.get("created_at"):
                            sections.append(
                                f"          <timestamp>{escape(comment['created_at'])}</timestamp>"
                            )
                ...
                sections.append("      <structured_data>")
                sections.append(
                    f"{self._indent_text(escape(json.dumps(thread_data, ensure_ascii=False, indent=2)), 8)}"
                )
                sections.append("      </structured_data>")
    ...
    def _extract_primary_tasks(self, analyzed_comments: AnalyzedComments) -> List[Dict[str, Any]]:
        ...
        for review in analyzed_comments.review_comments:
            for comment in review.actionable_comments:
                ...
                line_value = None
                if (
                    hasattr(comment, "line_range")
                    and comment.line_range
                    and self._is_valid_line_number(str(comment.line_range))
                ):
                    line_value = str(comment.line_range)
                elif (
                    hasattr(comment, "line_number")
                    and comment.line_number
                    and self._is_valid_line_number(str(comment.line_number))
                ):
                    line_value = str(comment.line_number)
                else:
                    # Try to extract line number from comment description
                    line_value = self._extract_line_number_from_description(
                        comment.issue_description
                    )

                # **REQUIREMENT FIX: Use actual GitHub comment ID instead of generated ID**
                comment_id = (
                    comment.comment_id if comment.comment_id else f"actionable_{actionable_counter}"
                )
                ...
                task = {
                    ...
                    "file": (
                        comment.file_path
                        if comment.file_path and len(comment.file_path) > 3
                        else "Unknown"
                    ),
                    "line": line_value,
                }
                ...
                if context_relationships:
                    task["context_strength"] = context_relationships.get(
                        "context_strengths", {}
                    ).get(comment.comment_id, 0.0)
                    task["description"] = self._enhance_description_with_context(
                        comment, context_relationships
                    )

                if change_analysis:
                    task["file_impact_score"] = change_analysis.get("file_impact_scores", {}).get(
                        comment.file_path, 0.0
                    )
                ...
                if comment.ai_agent_prompt:
                    ...
                    has_actual_code = (
                        ...
                        and comment.ai_agent_prompt.code_block.strip()
                        and comment.ai_agent_prompt.code_block
                        != comment.ai_agent_prompt.description
                        and not comment.ai_agent_prompt.code_block.strip().startswith("In ")
                    )
                    ...
                    if has_actual_code:
                        ...
                        language = (
                            ...
                            else (comment.ai_agent_prompt.language or "text")
                        )
            ...
            for comment in review.nitpick_comments:
                ...
                if change_analysis:
                    file_impact = change_analysis.get("file_impact_scores", {}).get(
                        comment.file_path, 0.0
                    )
                ...
                if (
                    hasattr(comment, "line_number")
                    and comment.line_number
                    and self._is_valid_line_number(comment.line_number)
                ):
                    line_value = comment.line_number
                elif hasattr(comment, "line_range") and self._is_valid_line_number(
                    comment.line_range
                ):
            ...
            for comment in review.outside_diff_comments:
                ...
                # **REQUIREMENT FIX: Use GitHub comment ID when available**
                comment_id = (
                    getattr(comment, "comment_id", None) or f"outside_diff_{outside_diff_counter}"
                )
    ...
    def _analyze_context_relationships(self, threads, actionable_comments) -> Dict[str, Any]:
        ...
        context_strengths = {}
        for _thread_id, mapping in relationships.get("thread_comment_mapping", {}).items():
    ...
    def _calculate_intelligent_priority(
        ...
        """
        original_priority = (
            comment.priority.value if hasattr(comment.priority, "value") else str(comment.priority)
        )
        ...
        context_strength = context_relationships.get("context_strengths", {}).get(
            comment.comment_id, 0.0
        )
        file_impact = change_analysis.get("file_impact_scores", {}).get(comment.file_path, 0.0)

        # Apply boosts
        current_priority_score = {"LOW": 1, "MEDIUM": 2, "HIGH": 3}.get(
            original_priority.upper(), 1
        )
    ...
    def _calculate_statistics(self, analyzed_comments: AnalyzedComments) -> Dict[str, int]:
        ...
        actionable_items = sum(
            ...
            + len(review.nitpick_comments)
            + len(review.outside_diff_comments)
            for review in analyzed_comments.review_comments
        )
        ...
        files_affected = len(
            {
                ...
                for review in analyzed_comments.review_comments
                for comment in (
                    review.actionable_comments
                    + review.nitpick_comments
                    + review.outside_diff_comments
                )
    ...
    def _should_skip_comment(self, content: str, file_path: str) -> bool:
        ...
        if file_path and any(
            noise in file_path.lower()
            for noise in ["(qb_new_en)", "unknown", "&gt;", "&lt;", "<details>", "<summary>"]
        ):
            return True
    ...
    def _generate_code_suggestion_from_comment(self, comment) -> str:
        ...
        if hasattr(comment, "line_number") and comment.line_number:
            ...
            full_description = (
                getattr(comment, "raw_content", "")
                or getattr(comment, "original_body", "")
                or description
            )
