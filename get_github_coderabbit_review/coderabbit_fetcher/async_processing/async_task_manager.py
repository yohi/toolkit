...
class AsyncTaskManager:
    ...
    def _create_execution_plan(self) -> List[List[str]]:
        ...
        while remaining_tasks:
            ...
            for task_id, task in remaining_tasks.items():
                if self._dependencies_satisfied(task, completed_tasks=set(self.tasks.keys()) - set(remaining_tasks.keys())):
    ...
    def _dependencies_satisfied(self, task: AsyncTask, completed_tasks: Optional[set] = None) -> bool:
        ...
        if completed_tasks is None:
            completed_tasks = {
                task_id for task_id, t in self.tasks.items()
        ...
        async with self.semaphore:
            ...
            try:
                ...
                if task.retry_attempts < task.max_retries:
                    ...
                    logger.info(f"Retrying task {task.name} (attempt {task.retry_attempts}/{task.max_retries})")
