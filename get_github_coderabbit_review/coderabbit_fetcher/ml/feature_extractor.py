...
class FeatureExtractor:
    ...
    def _init_word_lists(self) -> None:
        ...
        self.positive_words = {
            'good', 'great', 'excellent', 'nice', 'perfect', 'awesome',
            'fantastic', 'brilliant', 'clean', 'elegant', 'efficient',
            'smart', 'clever', 'neat', 'beautiful', 'love', 'like',
            'appreciate', 'thanks', 'thank', 'impressed', 'solid'
        }

        # Negative sentiment words
        self.negative_words = {
            'bad', 'terrible', 'awful', 'horrible', 'wrong', 'broken',
            'failed', 'error', 'hate', 'dislike', 'disappointed',
            'frustrated', 'confused', 'annoyed', 'messy', 'ugly',
            'inefficient', 'slow', 'bloated', 'convoluted'
        }
        ...
        self.technical_terms = {
            ...
            'vulnerability', 'authentication', 'authorization', 'encryption',
            'database', 'query', 'api', 'endpoint', 'interface', 'class',
            'method', 'function', 'variable', 'parameter', 'return',
            'exception', 'error', 'handling', 'validation', 'sanitization',
            'testing', 'coverage', 'mock', 'stub', 'integration',
            'deployment', 'ci', 'cd', 'build', 'compile', 'debug'
        }

        # Severity indicators
        self.critical_words = {
            'critical', 'urgent', 'severe', 'major', 'blocker',
            'security', 'vulnerability', 'breach', 'exploit'
        }

        self.minor_words = {
            'minor', 'nitpick', 'suggestion', 'consider', 'maybe',
            'optional', 'style', 'formatting', 'cosmetic'
        }
    ...
    def _init_patterns(self) -> None:
        ...
        # Security patterns
        self.security_pattern = r'(?i)\b(security|vulnerability|exploit|attack|injection|xss|csrf|auth)\b'

        # Performance patterns
        self.performance_pattern = r'(?i)\b(performance|slow|fast|optimization|bottleneck|memory|cpu|cache)\b'
    ...
    def _extract_context_features(self, context: Dict[str, Any]) -> ContextFeatures:
        ...
        is_config_file = any(ext in file_path.lower() for ext in ['.json', '.yaml', '.yml', '.toml', '.ini'])
        is_documentation = any(marker in file_path.lower() for marker in ['readme', 'doc', '.md', 'docs/'])
    ...
    def _extract_keyword_features(self, text: str) -> Dict[str, float]:
        ...
        # Severity keywords
        features['has_critical_words'] = float(any(word in text_lower for word in self.critical_words))
        ...
        features['critical_word_count'] = float(sum(1 for word in self.critical_words if word in text_lower))
        features['technical_density'] = float(len([word for word in text.split() if word.lower() in self.technical_terms])) / max(len(text.split()), 1)
    ...
    def _extract_pattern_features(self, text: str) -> Dict[str, float]:
        ...
        features['starts_with_question'] = float(text.strip().startswith(('What', 'Why', 'How', 'When', 'Where', 'Who')))
        features['ends_with_question'] = float(text.strip().endswith('?'))
        features['has_multiple_sentences'] = float(len(re.split(r'[.!?]+', text)) > 2)

        # Review patterns
        features['mentions_alternatives'] = float(bool(re.search(r'(?i)\b(alternative|instead|rather|consider|option)\b', text)))
        features['references_external'] = float(bool(re.search(r'(?i)\b(documentation|docs|spec|standard|best practice)\b', text)))
        features['suggests_testing'] = float(bool(re.search(r'(?i)\b(test|testing|unit test|integration test|coverage)\b', text)))
    ...
    def extract_batch_features(
        self,
        comments_with_context: List[Tuple[str, Optional[Dict[str, Any]]]]
        ...
        for comment_text, context in comments_with_context:
            try:
                ...
                feature_sets.append(CommentFeatureSet(
                    text_features=TextFeatures(),
                    context_features=ContextFeatures(),
                    keyword_features={},
                    pattern_features={}
    ...
    def get_feature_importance(
        self,
        feature_sets: List[CommentFeatureSet],
        labels: List[str]
    ) -> Dict[str, float]:
